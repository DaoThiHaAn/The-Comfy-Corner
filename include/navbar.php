...
<?php } else {?>
    ...
    <div class="dropdown">
        ...
        <div class="dropdown-content">
            <?php
            $categories = $mydatabase->query("SELECT name FROM category");
            while ($row = $categories->fetch_assoc()) {
                $category_name = ucwords($row['name']);
                echo "<a href=\"index.php?page=products&category={$row['name']}\">$category_name</a>";
            }
            ?>
        </div>
    </div>
    ...
    <div class="dropdown account-navbar">
        ...
        <div class="dropdown-content">
            ...
            <a onclick='openLogoutDialog()'>Logout</a>
        </div>
    </div>
